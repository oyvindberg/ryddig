...
trait LogActionsF[F[_]] extends Any {
  ...
  private inline def doLog[T: Formatter](t: => T, th: Option[Throwable], metadata: Metadata): F[Unit] =
    if metadata.logLevel >= minLogLevel then self(t, th, metadata) else unit
